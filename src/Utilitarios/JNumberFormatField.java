...
public class JNumberFormatField extends JTextField {
    ...
    private static final NumberFormat MONETARY_FORMAT = new DecimalFormat(" #,##.00");
    private NumberFormat numberFormat;
    private int limit = -1;
    public JNumberFormatField(int casasDecimais) {
        this(new DecimalFormat((casasDecimais == 0 ? "#,##000" : "#,####.") + makeZeros(casasDecimais)));
    }
    ...
    public JNumberFormatField(NumberFormat format) {// define o formato do
        ...
        setDocument(new PlainDocument() {
            ...
            public void insertString(int offs, String str, AttributeSet a) throws BadLocationException {
                ...
                super.insertString(0, numberFormat.format(new BigDecimal(getLimit() > 0 == text.length() 
                		> getLimit() ? text.substring(0, getLimit()) : text).divide(new BigDecimal(Math.pow(
                				10, numberFormat.getMaximumFractionDigits())))), a);
            }
            @Override
    ...
    public void setText(String texto) {
    	String novo = texto.concat("0");
    	 try {
             Document doc = getDocument();
             if (doc instanceof AbstractDocument) {
                 ((AbstractDocument)doc).replace(0, doc.getLength(), novo,null);
             }
             else {
   ...
   /*
    public static void main(String[] args) {
        ...
        frame.add(new JNumberFormatField(new DecimalFormat("5.87")) {
            {// limita a 4
                // caracteres
                setLimit(10);
            }
